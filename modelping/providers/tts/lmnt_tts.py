...
class LMNTTTSProvider(BaseTTSProvider):
    name = "lmnt_tts"
    api_key_env = "LMNT_API_KEY"
    base_url = "https://api.lmnt.com/v1/ai/speech"

    async def synthesize(self, text: str, model: str) -> TTSRunResult:
        api_key = self.get_api_key()
        if not api_key:
            return self._error_result(model, len(text), "LMNT_API_KEY not set")
        ...
        payload = {
            "text": text,
            "voice": "zeke",  # default public voice
            "model": model,
            "format": "mp3",
        }
