"""Groq provider — OpenAI-compatible API with streaming."""
...
class GroqProvider(BaseProvider):
    ...
    async def measure(self, model: str, prompt: str, max_tokens: int = 100) -> RunResult:
        ...
        if not api_key:
            return self._error_result(model, "GROQ_API_KEY not set")
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                async with client.stream(
                    ...
                    async for line in response.aiter_lines():
                        ...
                        # Groq includes x_groq.usage in final chunk
                        x_groq = chunk.get("x_groq", {})
                        if x_groq and x_groq.get("usage"):
                            usage = x_groq["usage"]
